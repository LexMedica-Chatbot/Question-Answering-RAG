...
def calculate_document_relevance(doc: Dict[str, Any], query: str) -> float:
    ...
    definition_terms = ['adalah', 'dimaksud dengan', 'yang disebut', 'pengertian', 'definisi']
...
def filter_documents_by_evaluation(documents: List[Dict[str, Any]], evaluation_result: str, query: str) -> List[Dict[str, Any]]:
    """
    ...
    if "KURANG MEMADAI" in evaluation_result.upper():
        ...
        print("[FILTER] Evaluation indicates documents are inadequate - keeping top documents for refinement")
        filtered_docs = [doc for score, doc in scored_docs[:3]]  # Keep top 3 for refinement
    else:
        # Evaluation says adequate - filter based on relevance
        print("[FILTER] Evaluation indicates documents are adequate - filtering by relevance")
        ...
        for score, doc in scored_docs:
            ...
            if score >= min_score and len(filtered_docs) < 5:  # Max 5 docs
                ...
                print(f"Status: {status}")
                print(f"Skor: {score:.3f}")
                print(f"Alasan: {'Skor terlalu rendah' if score < min_score else 'Melebihi batas maksimum dokumen'}")
...
def generate_answer(
    ...
    if query and evaluation_result:
        ...
        if query:
            scored_docs = [(calculate_document_relevance(doc, query), doc) for doc in doc_list]
            scored_docs.sort(key=lambda x: x[0], reverse=True)
    ...
    for i, doc in enumerate(doc_list, 1):
        ...
        print(f"[DEBUG] {ref_key} final metadata: status={status}, jenis={jenis}, nomor={nomor}, tahun={tahun}, tipe={tipe}")
    ...
    for doc in processed_docs:
        context_parts.append(f"{doc['ref_key']}: {doc['content'][:1000]}...")  # Increased content length
        reference_list.append(f"{doc['ref_key']} = {doc['citation']}")
    
    document_context = "\n\n".join(context_parts)
    reference_context = "\n".join(reference_list)
    
    # Enhanced prompt for focused citation
...
JAWABAN (dengan sitasi yang tepat untuk setiap informasi):"""
    ...
    try:
        ...
        print(f"[TOOL] 📊 Documents: {total_documents_processed} processed → {documents_used} used")
