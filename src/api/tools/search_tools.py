...
def search_documents(
    ...
    try:
        ...
        print(f"[SEARCH] 📄 Retrieved {len(docs)} documents from vector store")

        # Store all retrieved documents before filtering (for disharmony detection)
        all_retrieved_docs_data = []
        for i, doc in enumerate(docs):
            metadata = doc.metadata if isinstance(doc.metadata, dict) else {}
            content = clean_control(doc.page_content)
            ...
            # Extract info if metadata incomplete
            ...
            # Store ALL documents (including revoked ones) for disharmony detection
            all_retrieved_docs_data.append(
                {
                    "name": f"Dokumen #{i+1}",
                    "source": doc_name,
                    "content": content,
        ...
        for doc in docs:
            ...
            # Extract status from content if metadata incomplete
            if status == "berlaku" and not metadata.get("status"):
                content = clean_control(doc.page_content)
                extracted_info = extract_document_info(content)
                if extracted_info.get("status"):
                    status = extracted_info.get("status").lower()
        ...
        if eliminated_count > 0:
            print(
                f"[SEARCH] 🚫 Eliminated {eliminated_count} revoked documents, {len(active_docs)} remaining"
            )
        ...
        if not docs:
            return {
                "formatted_docs_for_llm": "Tidak ditemukan dokumen yang masih berlaku dan relevan dengan query tersebut.",
                "retrieved_docs_data": [],
                "all_retrieved_docs_data": all_retrieved_docs_data,  # Include all docs for disharmony detection
            }
        ...
        return {
            ...
            "retrieved_docs_data": [],
            "all_retrieved_docs_data": [],  # Empty list on error
        }
